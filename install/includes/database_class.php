...
class Database {
	...
	function create_tables($data)
	{
		...
		if ($mysqli->multi_query($query)) {
			...
			sleep(3);
			$mysqli = new mysqli($data['hostname'],$data['username'],$data['password'],$data['database']);
			$u=$data['user']; $e=$data['email']; $pp=$data['pwd']; $p=$this->hashpwd($data['pwd']); $d=date("d-m-Y");
			
			$sql = "INSERT INTO users (username,email,password,created_at, is_admin) VALUES ('$u','$e', '$p', '$d', TRUE)";
			$mysqli->query($sql);
			$mysqli->close();
			return true;
		}
